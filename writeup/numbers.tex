...
\begin{de}
  We define a natural number $a$ to be \term{divisible} by a natural number $b$ if there exists a natural number $c$ as follows:
...
\begin{pr}
  \term{Transitivity of divisibility}
...
\begin{de}
  \label{de:primes}
  A \term{prime number} is a number with precicely two distinct divisors: $1$ and itself.
\end{de}

\begin{de}
  A \term{prime factorisation} of a number $n\in\mathbb{N}$ is $n$ written as a product of powers of distinct primes:
  \[ n = \sum_{i}p_{i}^{m_i} \]
\end{de}
